...
export function MethodologySection({ colorClass = "bg-green-100", siteContent }: MethodologySectionProps) {
  ...
  return (
    <section className="relative my-24">
      ...
      >
        {methodology?.title || "My Design Methodology"}
      </div>
      ...
      <div className="sketch-card bg-white p-10 overflow-hidden">
        <div className="grid md:grid-cols-4 gap-4 relative">
          ...
          {methodology?.steps.map((step, index) => {
            ...
            const hoverColors = ["group-hover:bg-yellow-50", "group-hover:bg-blue-50", "group-hover:bg-green-50", "group-hover:bg-red-50"];
            
            return (
              <div key={index} className="relative z-10 flex flex-col items-center text-center gap-4 group">
                <div className={`w-16 h-16 rounded-full border-2 border-black bg-white flex items-center justify-center shadow-sm ${hoverColors[index % hoverColors.length]} transition-colors`}>
                  <Icon size={24} />
                </div>
        ...
        <div className="mt-12 p-4 border-2 border-black border-dashed bg-gray-200 text-[14px] leading-relaxed text-center italic">
          {methodology?.quote || '"I prioritize functionality and clarity. By staying in the low-fidelity phase longer, we ensure the structural integrity of the user experience before committing to visual aesthetics."'}
        </div>
      </div>
    </section>
  );
